...
function makeSession(overrides: Partial<Session> & { session_number: number; module: string }): SessionWithMeta {
  const data: Session = {
    title: `Session ${overrides.session_number}`,
    module: overrides.module,
    session_number: overrides.session_number,
    duration: 20,
    prerequisite: null,
  ...
  return {
    data,
    content: '',
    slug: `${String(overrides.session_number).padStart(2, '0')}-${overrides.module.toLowerCase().replace(/\s+/g, '-')}`,
  };
}
...
describe('groupByModule', () => {
  ...
  it('groups 3 sessions across 2 modules into 2 ModuleGroup objects', () => {
    const sessions = [
      makeSession({ session_number: 1, module: 'Oscillators' }),
      makeSession({ session_number: 2, module: 'Oscillators' }),
      makeSession({ session_number: 3, module: 'Filters' }),
    ];
    const result = groupByModule(sessions);
    expect(result).toHaveLength(2);
    expect(result[0].module).toBe('Oscillators');
    expect(result[0].sessions).toHaveLength(2);
    expect(result[1].module).toBe('Filters');
    expect(result[1].sessions).toHaveLength(1);
  });

  it('orders groups by first session_number, not alphabetically', () => {
    const sessions = [
      makeSession({ session_number: 5, module: 'Zebra' }),
      makeSession({ session_number: 1, module: 'Alpha' }),
      makeSession({ session_number: 3, module: 'Zebra' }),
    ];
    const result = groupByModule(sessions);
    // Alpha has session 1 (first), Zebra has session 3 (lowest in Zebra group)
    expect(result[0].module).toBe('Alpha');
    expect(result[1].module).toBe('Zebra');
  });

  it('preserves session order within each module (sorted by session_number)', () => {
    const sessions = [
      makeSession({ session_number: 1, module: 'Oscillators' }),
      makeSession({ session_number: 3, module: 'Oscillators' }),
      makeSession({ session_number: 2, module: 'Oscillators' }),
    ];
    const result = groupByModule(sessions);
...
describe('getAdjacentSessions', () => {
  ...
  it('crosses module boundaries (curriculum is linear)', () => {
    ...
    const result = getAdjacentSessions(sessions, sessions[1].slug);
    expect(result.next!.data.module).toBe('Filters');
  });
});
